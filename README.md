...
I've since changed over to ASP.NET in C#, which I worked with briefly in both my degree and most recent job. I've really enjoyed the change-over, and it's been a fun complement to a recent game-modding project in Java (also a repo on this account) but for something more professionally-aligned.
...
If you're here because I plugged this project in a job application, thank you for coming to have a look! Couple of comments:
- I've made a note below of where I've used AI in this project (I have cracked it open more than in the past). Besides that, I started the project by opening the "ASP.NET CORE + React" project template in Visual Studio -- so I have them to thank for some of the config (you can see what by starting a similar project). The rest is more or less my work, save the odd line copied from StackOverflow.
...
- 3 (lockedForModeration): present on a user's "Review" tab where it sits until they approve it to be published
- 4 (openForAddition): this segment is public on the website. It will turn up in searches when users go to join onto a story
- 5 (lockedForAddition): still public, but ensures that a segment may only have one segment being added to it at any time. (reverts to 4 when the segment being added is published.)
...
-  **ChainMates\ChainMates.Server** houses all other server code. This includes folders for Controllers, DTOs, and Services, split roughly according to which set of Objects they concern: Author, Story, Segment, Comment, and Notification. In terms of interesting code to look at, some notes:
...
Then in C#:
  ...
  e. From here, added features "natively" to the C# version (I'm not updating the DJango version for now).
...
- **Authentification**: I've been learning Auth set-up for this project, and while I used AI to generate some initial code and ideas, I've got enough of a grasp of it now that I'd like to take it to the next level purely based on my own understanding.
...
- **Config**: I've used AI to troubleshoot the Program.cs file when I was having trouble with it, making sure to ask it to actually explain / provide sources for any line it suggested adding.
